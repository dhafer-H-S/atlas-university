/**
 * make_report_pptx_light.js
 * Generates CacheLocalityLab_Presentation_Light.pptx  (bright / light mode)
 * Istanbul Atlas University | Computer Systems | Mid-term Project | Team 11
 *
 * Run from this directory:
 *   node make_report_pptx_light.js
 */
...
const OUT  = path.join(ROOT, "presentation", "CacheLocalityLab_C_Presentation.pptx");
...
const C = {
  ...
  panel:   "EEF2F7",   // very light blue-gray panels
  panel2:  "E2EAF4",   // slightly deeper panel
  border:  "93C5FD",   // soft blue border
  navy:    "0B2D52",   // deep navy — kept for header bars & accents
  teal:    "0D9488",   // darker teal (readable on white)
  blue:    "2563EB",   // deeper sky-blue
  gold:    "D97706",   // darker amber
  ...
  white:   "FFFFFF",   // white — for text on dark (navy) headers
  dark:    "0F172A",   // near-black — for titles/headings on white areas
};

// code block colours (light mode)
const CODE_BG   = "F1F5F9";   // very light gray
const CODE_TEXT = "1E3A5F";   // dark navy-blue code text
...
/** White background for every slide */
function baseSlide() {
...
/** Header bar — navy background with white title (readable on all slides) */
...
/** Rounded panel card — light fill with colored border */
...
/** Footer bar — navy with muted text */
function footer(s, text) {
  ...
  s.addText(text, {
    ...
    fontSize: 8, color: "93C5FD", fontFace: FONT.body, valign: "middle",
  });
}
...
// ═══════════════════════════════════════════════════════════════════════════
{
  ...
  levels.forEach(lv => {
    ...
    s.addText(lv.sub, {
      ...
      fontSize: 8.5, color: "E0F2FE", fontFace: FONT.body,
    });
    ty += 0.66;
  });
  ...
  // main title — dark text on white background
  s.addText("Cache & Locality", {
    x: 0.45, y: 1.4, w: 7.9, h: 0.85,
    fontSize: 44, bold: true, color: C.dark, fontFace: FONT.title,
  });
...
// ═══════════════════════════════════════════════════════════════════════════
{
  ...
  const items = [
    { num: "01", title: "Methodology",       desc: "Timer infrastructure · CLOCK_NOW macro · clock_gettime(CLOCK_MONOTONIC) · platform notes for Apple Silicon",      color: C.blue },
  ...
  items.forEach((it, i) => {
    ...
    s.addShape(pres.shapes.ROUNDED_RECTANGLE, {
      x: col, y, w, h,
      fill: { color: C.panel }, line: { color: it.color, pt: 1.5 }, rectRadius: 0.08,
    });
    s.addShape(pres.shapes.RECTANGLE, {
    ...
    s.addText(it.title, {
      ...
      fontSize: 13, bold: true, color: C.dark, fontFace: FONT.title,
    });
    s.addText(it.desc, {
  ...
  footer(s, "Five benchmark modules  ·  C11  ·  gcc -O2  ·  Apple Silicon M-series");
}
...
// ═══════════════════════════════════════════════════════════════════════════
{
  ...
  s.addShape(pres.shapes.RECTANGLE, {
    ...
    fill: { color: CODE_BG }, line: { color: C.border },
  });
  s.addText(
    `/* timer.h — POSIX clock_gettime wrapper */\n#include <time.h>\nstatic inline double timespec_diff_ns(\n    const struct timespec *t0,\n    const struct timespec *t1) {\n  return (double)(t1->tv_sec  - t0->tv_sec)*1e9\n       + (double)(t1->tv_nsec - t0->tv_nsec);\n}\n#define CLOCK_NOW(ts) clock_gettime(CLOCK_MONOTONIC,&(ts))\n\n/* Usage in each benchmark: */\ndouble best_ns = 1e18;\nfor (int iter = 0; iter < BENCH_ITERS; iter++) {\n  struct timespec t0, t1;\n  CLOCK_NOW(t0);\n  /* ... benchmark kernel ... */\n  CLOCK_NOW(t1);\n  double ns = timespec_diff_ns(&t0,&t1) / n;\n  if (ns < best_ns) best_ns = ns;\n}`,
    {
      x: 0.68, y: 1.72, w: 5.72, h: 3.0,
      fontSize: 9, color: CODE_TEXT, fontFace: FONT.mono,
  ...
  s.addText(
    "src/  timer.h  config.h  main.c\n"
    + "     stride_bench.c   workingset_bench.c\n"
    + "     matrix_bench.c   linkedlist_bench.c   bandwidth_bench.c",
    {
      x: 0.58, y: 6.38, w: 5.48, h: 0.58,
      fontSize: 8.2, color: CODE_TEXT, fontFace: FONT.mono,
  ...
  s.addText(
    ...
    + "$ gcc -O2 -std=c11 -Wall -Wextra src/*.c -o cache_lab",
    {
  ...
  footer(s, "Compiler: gcc -O2 -std=c11 -Wall -Wextra  ·  BENCH_ITERS=5  ·  MIN_ACCESSES=4,000,000");
}
...
// ═══════════════════════════════════════════════════════════════════════════
{
  ...
  // large chart image
  s.addImage({ path: fig("stride.png"), x: 0.25, y: 1.12, w: 9.1, h: 4.05 });

  // insight panels (right)
  ...
  s.addText("Benchmark Kernel  —  stride_bench.c", {
  ...
  s.addText(
    `for (long long i = 0; i < SIZE; i += stride_elems)\n  sum += arr[i];      // touches a new 64-B cache line\n                      // whenever stride_bytes >= 64\n\n// volatile sink prevents dead-code elimination:\nvolatile long long _sink = sum;`,
    {
      ...
      fontSize: 8.5, color: CODE_TEXT, fontFace: FONT.mono,
      lineSpacingMultiple: 1.2, wrap: false,
    }
  );
...
// ═══════════════════════════════════════════════════════════════════════════
{
  ...
  s.addImage({ path: fig("workingset.png"), x: 0.25, y: 1.12, w: 8.7, h: 4.0 });
  ...
  s.addText("Sequential Scan Kernel  —  workingset_bench.c", {
...
// ═══════════════════════════════════════════════════════════════════════════
{
  ...
  s.addImage({ path: fig("matrix.png"), x: 0.25, y: 1.12, w: 9.0, h: 4.1 });
  ...
  s.addText("The Critical Difference  —  matrix_bench.c", {
  ...
  s.addText(
    `// ✓ Row-major — cache-friendly (stride = 8 B / double)\nfor (int i=0;i<N;++i)\n  for (int j=0;j<N;++j) s += m[i][j];\n\n// ✗ Col-major — cache-hostile (stride = N×8 B)\nfor (int j=0;j<N;++j)\n  for (int i=0;i<N;++i) s += m[i][j];   // 1.5× slower!`,
    {
      ...
      fontSize: 8.4, color: CODE_TEXT, fontFace: FONT.mono,
      lineSpacingMultiple: 1.2, wrap: false,
    }
  );
...
// ═══════════════════════════════════════════════════════════════════════════
{
  ...
  s.addImage({ path: fig("linkedlist.png"), x: 0.25, y: 1.12, w: 8.8, h: 4.05 });
  ...
  s.addText("Node Struct & Chase Loop  —  linkedlist_bench.c", {
    x: 0.48, y: 5.36, w: 5.7, h: 0.26,
  ...
  s.addText(
    `struct __attribute__((aligned(64))) Node {\n  struct Node *next;\n  char pad[56]; /* 1 node = 1 cache line */\n};\n// Fisher-Yates shuffle with srand(42) + rand()\n__attribute__((noinline))\nNode* chase_nodes(Node *head, long long n) {\n  for (Node* p = head; p; p = p->next);\n  __asm__ volatile("" ::: "memory"); /* barrier */\n}`,
    {
      x: 0.58, y: 5.68, w: 5.5, h: 1.17,
      fontSize: 8.2, color: CODE_TEXT, fontFace: FONT.mono,
      lineSpacingMultiple: 1.2, wrap: false,
...
// ═══════════════════════════════════════════════════════════════════════════
{
  ...
  s.addImage({ path: fig("bandwidth.png"), x: 0.25, y: 1.12, w: 9.05, h: 4.05 });
  ...
  s.addText("Two Kernels, Two Bottlenecks  —  bandwidth_bench.c", {
    x: 0.48, y: 5.36, w: 5.7, h: 0.26,
...
// ═══════════════════════════════════════════════════════════════════════════
// SLIDE 9 — RESULTS DASHBOARD (2×2 grid)
// ═══════════════════════════════════════════════════════════════════════════
{
  const s = baseSlide();
  ...
  const imgs = [
    { f: "stride.png",      label: "Exp 1 — Stride Access",      x: 0.25,  y: 1.12 },
    { f: "workingset.png",  label: "Exp 2 — Working Set",        x: 6.72,  y: 1.12 },
    { f: "matrix.png",      label: "Exp 3 — Matrix Traversal",   x: 0.25,  y: 4.25 },
    { f: "bandwidth.png",   label: "Exp 5 — Memory Bandwidth",   x: 6.72,  y: 4.25 },
  ];
  imgs.forEach(img => {
    s.addImage({ path: fig(img.f), x: img.x, y: img.y, w: 6.35, h: 3.02 });
    s.addText(img.label, {
      x: img.x, y: img.y + 2.83, w: 6.35, h: 0.22,
      fontSize: 7.5, italic: true, color: C.muted, fontFace: FONT.body, align: "center",
    });
  });
...
// ═══════════════════════════════════════════════════════════════════════════
{
  ...
  const findings = [
    {
      ...
      code: "struct __attribute__((aligned(64))) HotData { int x, y, z, pad[13]; };",
      color: C.teal,   badge: "CCFBF1",
    },
    {
      n: "2",
  ...
  findings.forEach((f, i) => {
    ...
    s.addShape(pres.shapes.ROUNDED_RECTANGLE, {
      x, y, w, h,
      fill: { color: C.panel }, line: { color: f.color, pt: 1.5 }, rectRadius: 0.1,
    });
    s.addShape(pres.shapes.RECTANGLE, { x, y, w: 0.1, h, fill: { color: f.color }, line: { color: f.color } });

    // number badge (pastel fill with accent border)
    ...
    // code hint box (light bg, dark code text)
    s.addShape(pres.shapes.RECTANGLE, {
      x: x + 0.18, y: y + 1.89, w: w - 0.3, h: 0.4,
      fill: { color: CODE_BG }, line: { color: f.color, pt: 0.8 },
    });
    s.addText(f.code, {
      x: x + 0.24, y: y + 1.91, w: w - 0.42, h: 0.36,
      fontSize: 7.6, color: CODE_TEXT, fontFace: FONT.mono, valign: "middle", wrap: false,
    });
  });

  footer(s, "All findings supported by live benchmark data collected on Apple Silicon M-series  ·  C11  ·  gcc -O2");
}
...
// ═══════════════════════════════════════════════════════════════════════════
{
  ...
  // full-bleed decorative left block — light panel
  s.addShape(pres.shapes.RECTANGLE, {
    x: 0, y: 0, w: 5.6, h: H,
    fill: { color: C.panel }, line: { color: C.panel },
  });
  ...
  s.addText("Conclusions", {
    ...
    fontSize: 28, bold: true, color: C.dark, fontFace: FONT.title,
  });
  s.addShape(pres.shapes.RECTANGLE, {
  ...
  s.addText("Cache & Locality Performance Lab", {
    ...
    fontSize: 15, color: C.dark, fontFace: FONT.title, align: "center",
  });
  s.addText("Computer Systems  ·  Istanbul Atlas University  ·  April 2026", {
  ...
  s.addText("Questions?", {
    ...
    fontSize: 18, bold: true, color: C.dark, fontFace: FONT.title, align: "center",
  });
}
