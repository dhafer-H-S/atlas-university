...
def build_doc(figs):
    ...
    add_body(doc,
        ...
        "random-access latency, and sustained memory bandwidth. All benchmarks were implemented in C11, "
    ...
    add_body(doc,
        "Every experiment shares a single timing primitive defined in src/timer.h. The macro "
        "CLOCK_NOW(ts) records wall-clock time using clock_gettime(CLOCK_MONOTONIC), which "
        "provides nanosecond resolution on POSIX systems without requiring any C++ standard "
        "library headers. A companion inline function timespec_diff_ns() computes the elapsed "
        "time in nanoseconds between two struct timespec snapshots.")
    add_body(doc,
        "Each benchmark wraps its kernel in a best-of-five timing loop. The loop records the "
        "minimum elapsed time divided by the access count. Choosing the minimum rather than "
        "the mean removes the effect of OS scheduling preemptions, page-fault stalls, and thermal "
        "throttling events, all of which introduce upward outliers. The timed block is placed "
        "inline in each benchmark function rather than passed as a callback.")
    add_code_block(doc,
        ...
        "    /* ... benchmark kernel ... */\n"
        "    CLOCK_NOW(t1);\n"
        "    double ns = timespec_diff_ns(&t0, &t1) / (double)n_accesses;\n"
        "    if (ns < best_ns) best_ns = ns;\n"
        "}"
    )
    add_caption(doc, "Programme 2.1  Timing infrastructure from src/timer.h")
    ...
    add_body(doc,
        ...
        "the data is present in at least the L3 cache before timing begins. Second, the timed block "
        "records the kernel's elapsed time using CLOCK_NOW. Third, results are accumulated "
    ...
    add_body(doc,
        ...
        "The compiler flags -O2 -std=c11 -Wall -Wextra are fixed for all translation units. "
    ...
    add_body(doc,
        "C stores two-dimensional arrays in row-major order: the elements of row i occupy "
    ...
    add_body(doc,
        ...
        "enforced by the __attribute__((aligned(64))) attribute and a 56-byte padding array. After "
        "allocating N nodes with aligned_alloc(64, ...), a Fisher-Yates shuffle using srand(42) and "
        "rand() produces a random permutation of node indices, and the benchmark traverses the list "
        "by following next pointers. Because each successive access address is unknown until the "
        "previous load completes, the CPU cannot issue speculative prefetch requests and must wait "
        "for the full memory latency at every hop.")
    add_body(doc,
    ...
    add_caption(doc, "Programme 7.1  GB/s conversion formula from src/bandwidth_bench.c")
    ...
    add_body(doc,
        ...
        "and quantified with sub-nanosecond precision using only standard C11 and the "
        "POSIX clock_gettime interface. The key findings are summarised below.")
    ...
    refs = [
        ...
        "ISO/IEC, Programming Languages — C, ISO/IEC 9899:2011 (C11), 2011.",
    ]
